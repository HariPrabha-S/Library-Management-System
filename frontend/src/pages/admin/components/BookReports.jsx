                ...
                <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 overflow-y-auto bg-gray-50/50">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="border border-gray-200 p-4 rounded-xl bg-white shadow-sm relative pt-5">
                            <span className="absolute -top-3 left-3 bg-[var(--color-primary)] text-white px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider shadow-sm">Search Criteria</span>
                            <div className="grid grid-cols-2 gap-2 mt-1">
                                {searchCriteria.map(item => (
                                    <label key={item.value} className="flex items-center gap-2 text-xs cursor-pointer group">
                                        <div className="relative flex items-center justify-center">
                                            <input
                                                ...
                                                className="sr-only"
                                            />
                                            <div className={`w-3.5 h-3.5 rounded-full border-2 transition-all duration-200 ${localFilters.field === item.value ? "border-[var(--color-secondary)] scale-110" : "border-gray-300 group-hover:border-gray-400"}`}></div>
                                            {localFilters.field === item.value && (
                                                <div className="absolute w-1.5 h-1.5 rounded-full bg-[var(--color-secondary)] animate-in fade-in zoom-in duration-200"></div>
                                            )}
                                        </div>
                                        <span className={`transition-colors duration-200 truncate ${localFilters.field === item.value ? "text-gray-900 font-semibold" : "text-gray-600 font-medium group-hover:text-gray-800"}`}>
                                            {item.label}
                                        </span>
                                    </label>
                                ))}
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-3">
                                <div className="flex flex-col gap-1">
                                    <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tight">From Purchase Date</span>
                                    <input
                                        type="date"
                                        value={localFilters.fromDate || ""}
                                        onChange={(e) => setLocalFilters({ ...localFilters, fromDate: e.target.value })}
                                        className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:ring-1 focus:ring-[var(--color-secondary)] outline-none"
                                    />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tight">To Purchase Date</span>
                                    <input
                                        type="date"
                            ...
                            <button
                                onClick={handleLoad}
                                className="mt-4 w-full bg-[var(--color-secondary)] text-white hover:opacity-90 py-2 text-xs font-bold rounded-lg shadow-md transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                            >
                                <FiEye size={12} />
                                Preview Report
                            </button>
                    ...
                    <div className="space-y-4">
                        ...
                        <button
                            className="w-full flex items-center gap-2 text-[10px] bg-[var(--color-secondary)]/5 p-2.5 rounded-lg border border-[var(--color-secondary)]/10 cursor-pointer hover:bg-[var(--color-secondary)]/10 transition-colors"
                        >
                            <input type="checkbox" checked readOnly className="w-3 h-3 rounded accent-[var(--color-secondary)]" />
                            <span className="font-semibold text-[var(--color-secondary)]">Show Selection Condition on Report</span>
                        </button>
                    </div>
                </div>
