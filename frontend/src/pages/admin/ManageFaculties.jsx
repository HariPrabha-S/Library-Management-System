...
export default function ManageFaculty() {
    ...
    const fetchFaculty = async () => {
        try {
            setLoading(true);
            const res = await adminService.getFaculties({ search, department: departmentFilter });
            if (res.success) {
                setFaculty(res.data.map(f => ({ ...f, _id: f.id })));
            }
        } catch (error) {
    ...
    const processedFaculty = [...faculty].sort((a, b) => {
        switch (sortOption) {
            case "name": return a.name.localeCompare(b.name);
            case "facultyId": return (a.facultyId || "").localeCompare(b.facultyId || "");
    ...
    return (
        <>
            ...
            {selectedFaculty.length > 0 && (
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-white border border-gray-200 px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-6 animate-in slide-in-from-bottom duration-300 z-50">
                    <span className="text-sm font-bold text-gray-600 tracking-tight">
                        <span className="text-(--color-primary)">{selectedFaculty.length}</span> faculty selected
                    </span>
                    <button onClick={async () => {
                        if (window.confirm(`Delete ${selectedFaculty.length} faculty?`)) {
                            try {
                                await adminService.deleteBulkFaculties(selectedFaculty);
                                setSelectedFaculty([]);
                                fetchFaculty();
                            } catch (e) { alert("Failed to delete faculty"); }
                        }
            ...
            {showModal && <AddFacultyModal setShowModal={setShowModal} refreshFaculties={fetchFaculty} />}
