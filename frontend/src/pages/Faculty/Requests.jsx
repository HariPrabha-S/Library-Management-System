...
import { Clock, CheckCircle, XCircle, BookOpen, CalendarDays, RefreshCw, AlertCircle, Ban } from 'lucide-react';
...
const Requests = ({ user }) => {
  ...
  const facultyEmployeeId = user?.facultyId || loggedInUser.facultyId || 'NSCIT001';
  ...
  const fetchData = useCallback((showLoader = false) => {
    ...
    fetch(`/api/requests/${facultyEmployeeId}`, { headers: authHeaders })
      .then(r => r.json())
      .then(data => {
    ...
    fetch(`/api/reservations/my-reservations?memberId=${memberId}&memberType=Faculty`, { headers: authHeaders })
      .then(r => r.json())
      .then(res => {
  ...
  const handleCancelReservation = async (reservationId) => {
    ...
    try {
      ...
      const res = await fetch(`/api/reservations/cancel/${reservationId}`, {
        ...
        body: JSON.stringify({ memberId, memberType: 'Faculty' })
      });
