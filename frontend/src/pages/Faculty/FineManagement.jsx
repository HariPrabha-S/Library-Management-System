...
const FineManagement = ({ user }) => {
  ...
  const fetchFines = () => {
    ...
    setLoading(true);
    fetch(`/api/borrowing-info/${encodeURIComponent(user.facultyId)}`)
      .then(r => {
      ...
      <div className="panel" style={{ marginBottom: 24 }}>
        <div className="panel-header">
          ...
          <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>
            Note: Late return fines do not apply to faculty
          </span>
        </div>
              ...
              {data.details?.map((fine) => (
                <tr key={fine.id}>
                  ...
                  <td style={{ color: 'var(--text-secondary)', fontSize: '0.87rem' }}>
                    {fine.isLost ? (
                      <span style={{ color: 'var(--danger)', fontWeight: 600 }}>Lost Book (2× price)</span>
                    ) : (
